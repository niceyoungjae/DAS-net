"""DeepSupDyUNet: ThinDyUNet + Deep Supervision."""
import torch
...
class DecoderDeepSup(nn.Module):
    """Decoder with Deep Supervision: auxiliary outputs from intermediate stages."""
    def __init__(self, in_channels, out_channels, num_class, padding, size=6, upsample=False):
        ...
        for i in range(size):
            ...
            # Auxiliary head for deep supervision (last 3 stages)
            if i >= size - 3:
    ...
    def forward(self, x, routes_connection, return_aux=False):
        ...
        for layer in self.decoder_layers:
            if isinstance(layer, MultiCNNBlock):
                ...
                x = layer(x)
                # Collect auxiliary output if available
                if return_aux and block_idx < len(self.aux_heads) and self.aux_heads[block_idx] is not None:
                    aux_outputs.append(self.aux_heads[block_idx](x))
                block_idx += 1
            else:
                x = layer(x)
...
class DeepSupDyUNet(nn.Module):
    """ThinDyUNet with Deep Supervision."""
